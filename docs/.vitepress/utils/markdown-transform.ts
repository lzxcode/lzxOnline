...
import { docRoot, docsDirName, projRoot } from "./paths";
import { getLang, languages } from "../utils/lang";
import footerLocale from "../json/pages/footer.json";
  ...
  return {
    ...
    async transform(code, id) {
      ...
      const append: Append = {
        ...
        footers: [],
        scriptSetups: [
          `const demos = import.meta.globEager('../../examples/${componentId}/${componentId}/*.vue')`,
        ],
      };
...
const transformComponentMarkdown = (
  ...
  const docUrl = `www./${docsDirName}/mdFile/component/${componentId}.md`;
  const componentUrl = `www./packages/components/${componentId}`;
  const componentPath = path.resolve(
    projRoot,
    `docs/examples/components/${componentId}`
  );
